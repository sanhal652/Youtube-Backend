...
const addComment = asyncHandler(async (req, res) => {
    ...
    const commentKeys = await client.keys(`video_comments:${videoId}:*`)
    if (commentKeys.length > 0)
        await client.del(commentKeys)
...
const updateComment = asyncHandler(async (req, res) => {
    ...
    const commentKeys = await client.keys(`video_comments:${comment.video}:*`)
    if (commentKeys.length > 0)
        await client.del(commentKeys)
